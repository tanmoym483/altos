  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (arrayValue.length == 0) {
        ...
        if(sponser.wallet == undefined){
          $('#customer_wallet_amount').text('In wallet'+' Rs:0');
        } else{
          $('#customer_wallet_amount').text('In wallet'+' Rs:'+sponser.wallet);
        }
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (arrayValue.length == 0) {
        ...
        $('#franchise_wallet_amount').text('In wallet'+' '+sponser.wallet);
      }
    },
    error: function (data) {
      $("#name").val("");
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (arrayValue.length == 0) {
        ...
        var membarName = '';
        if(Object.prototype.hasOwnProperty.call(userdata[0], "middleName")){
           membarName =
        userdata[0].firstName + ' '+ userdata[0].middleName + ' '+ userdata[0].lastName;
        } else {
          membarName =
        ...
        $("#firstName").val(JSON.parse(response)[0].firstName).prop("readonly", true);
        $("#errorMessage").html("");
        $("#middleName").val(JSON.parse(response)[0].middleName).prop("readonly", true);
        $("#errorMessage").html("");
        $("#lastName").val(JSON.parse(response)[0].lastName).prop("readonly", true);
        $("#errorMessage").html("");
        $("#mobile").val(JSON.parse(response)[0].phone).prop("readonly", true);
        $("#errorMessage").html("");
        $("#birthday").val(JSON.parse(response)[0].birthday).prop("readonly", true);
        $("#errorMessage").html("");
        $("#accountHolderName").val(membarName).prop("readonly", true);
        $("#errorMessage").html("");
        $("#emailid").val(JSON.parse(response)[0].email).prop("readonly", true);
        $("#errorMessage").html("");
        $("#district").val(JSON.parse(response)[0].district).prop("readonly", true);
        $("#errorMessage").html("");
        $("#pincode").val(JSON.parse(response)[0].postcode).prop("readonly", true);
        $("#errorMessage").html("");
        $("#state").val(JSON.parse(response)[0].state).prop("readonly", true);
        $("#errorMessage").html("");
        $("#city").val(JSON.parse(response)[0].city).prop("readonly", true);
        $("#errorMessage").html("");
        $("#address").val(JSON.parse(response)[0].address).prop("readonly", true);
        $("#errorMessage").html("");
...
function postcodeDynamic() {
  ...
  if(pinId.length && pinId.length % 6 == 0){
  $.ajax({
...
function userdetailsUpdate(userId) {
  $.ajax({
    ...
    success: function (response) {
      ...
      $("#birthday").val(sponser.birthday);
      $("#city-2").val(sponser.city);
      $("#state").val(sponser.state);
      $("#postcode-2").val(sponser.postcode);
      $("#address-2").val(sponser.address);
      $("#status").val(sponser.ustatus);
      $("#transRefNo-2").val(sponser.transRefNo);
      $("#transactionid").val(sponser.transactionid);
      if(sponser.ustatus == 'reject'){
        $('#reject-portion').show();
      } else {
        $('#reject-portion').hide();
      }
