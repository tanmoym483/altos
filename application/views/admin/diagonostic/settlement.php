            ...
            <div class="row mb-2">
                ...
                <?php if($_GET['to'] == ''){ ?>
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Settlement Amount</li>
                    </ol>
                    <?php } else { ?>
                    <ol class="breadcrumb float-sm-right">
                    ...
                    <div class="table-responsive" >
                            <table class="table table-bordered exportable"  id="healthcard_transaction" style="font-size:14px" data-ordering="false">
                                ...
                                <?php if (!empty($data)) { ?>
                                       ...
                                       if($customer->payment_status == 'full payment'){
                                       // $amount = ($customer->total_amount * $usermember->commision)/100;
                                       // $due = $customer->total_amount - $customer->paidamount;
                                       // $amount = $due - $_amount;
                                       
                                        $mode = 'debited';
                                        $withdraw = $customer->test_commision;
                                        $deposite = 0;
                                        $total = $total - $withdraw;
                                       } 
                                       if($customer->payment_status == 'on emi'){
                                        //$due = $due_amount;
                                        $_amount = $customer->test_commision;
                                        $due = $customer->total_amount - $customer->paidamount;
                                       // $amount = $total - $_amount;
                                        ...
                                        $total = $total - $_amount;
                                        $this->db->select('test_category')->from('diagonostic_commision')->where('id',$customer->test_category);
                                        $query = $this->db->get();
                                       ...
                                       $this->db->select('center_name')->from('diagonostic_member')->where('userId',$customer->created);
                                       $query1 = $this->db->get();
                                       $center = $query1->row();
                                       ...
                                       <tr>
                                        ...
                                        <td><?=$mode?></td>
                                        <td><?=$name?> - <?=$customer->cardnumber?> transaction: <?=$customer->transRefno?> Test: <?=$tests->test_category?> Center: <?=$center->center_name?> </td>
                                        <td><?=round($deposite)?></td>
                                        <td><?=round($withdraw)?></td>
                                        <td><?=abs(round($total))?></td>
                                       </tr>
